...
Run with Hyper-V VM/storage permissions as the invoking interactive user, never as `LocalSystem`; local `Hyper-V Administrators` membership suffices without elevation. Host session 0 is only an unattended-host convenience against PowerShell Direct credential UI, is unrelated to the guest desktop session the proof needs, and diverges profile, `PATH`, and SSH known-hosts. Doctor is non-mutating inventory/feature proof only: it does not boot the image or validate credentials, KVP/VSS, or browsers.
