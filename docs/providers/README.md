...
| [hyperv](hyperv.md) | built-in; `ssh-lease` · local-vm | Crabbox-managed SSH; `crabbox-sync` · direct only; features: `ssh`, `crabbox-sync`, `desktop`, `browser`, `cleanup`, `pause-resume`, `tailscale`, `cache-volume`, `workspace-checkpoint`, `workspace-fork`, `workspace-restore`, `provider-snapshot`; target differences: `linux` adds `code` | `linux`, `windows/normal`; Microsoft Hyper-V VM | `local`; GPU: no | Crabbox; Exact-claim VM delete; cache VHDXs persist | Local Linux or native Windows Generation 2 VM | Windows Hyper-V host, VM-management permissions, and target-ready VHDX required |
